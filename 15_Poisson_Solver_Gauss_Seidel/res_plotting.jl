...
ax3.semilogy(mg_iter_hist, mg_res_hist[:,2],color="green",
            lw=4,label="Multigrid framework")
ax3.set_xlabel("Iteration count")
ax3.set_ylabel("\$|r|_2\$")
ax3.legend()

ax4.semilogy(gs_iter_hist, gs_res_hist[:,2],color="blue",
            lw=4,label="Gauss-Seidel method")
ax4.semilogy(cg_iter_hist, cg_res_hist[:,2],color="orange",
            lw=4,label="Conjugate-Gradient method")
ax4.semilogy(mg_iter_hist, mg_res_hist[:,2],color="green",
            lw=4,label="Multigrid framework")
ax4.set_xlim(0,2000)
